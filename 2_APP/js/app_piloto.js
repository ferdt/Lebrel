...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Actualiza la hora local independiente de la telemetría, aunque idealmente se manda o compensa desde backend.
    setInterval(() => {
        const now = new Date();
        ui.hora.textContent = now.toLocaleTimeString('es-ES', { hour12: false });
    }, 1000);

    client.onMessage((data) => {
        if (data.tramo_nombre !== undefined) ui.tramo_nombre.textContent = data.tramo_nombre;
        
        if (data.distancia_m !== undefined) {
            ui.distancia.textContent = (data.distancia_m / 1000).toFixed(3) + ' km';
        }
        ...
        if (data.velocidad_kmh !== undefined) ui.velocidad_actual.textContent = data.velocidad_kmh.toFixed(1);
        if (data.velocidad_objetivo_kmh !== undefined) ui.velocidad_objetivo.textContent = data.velocidad_objetivo_kmh.toFixed(1);

        if (data.proxima_media_kmh !== undefined) {
            ui.proxima_media.textContent = data.proxima_media_kmh.toFixed(1) + ' km/h';
        }
        if (data.distancia_cambio_m !== undefined) {
